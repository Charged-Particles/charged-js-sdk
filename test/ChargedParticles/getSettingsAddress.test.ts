import { getSettingsAddress } from '../../src/ChargedParticles';
...
const mainnetAddy = mainnetAddresses.chargedSettings.address;
const kovanAddy = kovanAddresses.chargedSettings.address;
const polygonAddy = polygonAddresses.chargedSettings.address;
const mumbaiAddy = mumbaiAddresses.chargedSettings.address;
...
describe('getStateAddressTest', () => {
  it('return correct address with empty params.', async () => {
    expect(await getSettingsAddress()).toEqual(mainnetAddy);
  })
})
...
describe('network string (using alchemy)', () => {
  ...
  it('should return mainnet state address with network string given. using alchemy api', async () => {
    expect(await getSettingsAddress(alcProvider)).toEqual(mainnetAddy);
  })
})
...
describe('alchemy kovan', () => {
  ...
  it('should return kovan state address with alchemy provider given', async () => {
    expect(await getSettingsAddress(alcProvider, 'kovan')).toEqual(kovanAddy);
  })
})
...
describe('alchemy polygon', () => {
  ...
  it('should return polygon state address with alchemy provider given', async () => {
    expect(await getSettingsAddress(alcProvider, 'matic')).toEqual(polygonAddy);
  })
})
...
describe('alchemy mumbai', () => {
  ...
  it('should return mumbai state address with alchemy provider given', async () => {
    expect(await getSettingsAddress(alcProvider, 'maticmum')).toEqual(mumbaiAddy);
  })
})
